...
if __name__ == "__main__":
    ...
    step_sizes = [15]
    training_losses = np.zeros((len(learning_rates),len(step_sizes),epochs))
    validation_losses = np.zeros((len(learning_rates),len(step_sizes),epochs))
    best_loss = 100
    best_dict = dict()
    for i,lr in enumerate(learning_rates):
        for j,stp_size in enumerate(step_sizes):
          ...
          train_los,val_los = trainer.run_model(model,loss_fn,optimizer,(train_params_scaled,train_gamma),(val_params_scaled,val_gamma),
